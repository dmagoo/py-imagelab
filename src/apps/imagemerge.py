...
class App:
    ...
    def initialize_display(self):
        ...
        self.screen = pygame.display.set_mode(
            SCREENRECT.size, pygame.RESIZABLE, self.bit_depth
        )
    ...
    def validate_input_files(self, input_file_list):
        unflattened = [
            ...
            if is_image_file(path)
                ...
                file for file in path.iterdir() if is_image_file(file) and path.is_dir()
            ]
    ...
    def run_merger(self):
        ...
        for path in self._input_files:
            ...
            if dimensions is None:
                ...
                self.screen = pygame.display.set_mode(
                    dimensions, pygame.RESIZABLE, self.bit_depth
                )
            current_dimensions = surface.get_size()
            if current_dimensions != dimensions:
                raise ValueError(f"Dimension Mismatch for file: {path.resolve()}")
            surface.convert()
