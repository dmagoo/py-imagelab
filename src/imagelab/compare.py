...
def match_score(source, candidate, clip_rect=None):
    ...
    if clip_rect:
        source_array = pygame.surfarray.array3d(
            source.subsurface(source.get_rect().clip(clip_rect)).copy()
        ).astype(int)
        candidate_array = pygame.surfarray.array3d(
            candidate.subsurface(candidate.get_rect().clip(clip_rect)).copy()
        ).astype(int)
    else:
